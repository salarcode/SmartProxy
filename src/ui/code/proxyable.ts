...
export class proxyable {
    static grdProxyable: any;
    static sourceTabId: number = null;
    static selfTabId: number;
    static proxyableData: ProxyableInternalDataType;
    ...
    static populateDataForProxyable(dataForProxyable: ProxyableInternalDataType) {
        if (!dataForProxyable)
            return;
    ...
    static loadRequests(requests: ProxyableDataType[]) {
    ...
    static refreshGridAllRows(invalidate?: boolean) {
        if (invalidate) {
    ...
    static onDisableSubdomainClick() {
        let element = jQuery(this);
        let domain = element.data("domain");
        if (!domain)
            return;
    ...
    static toggleProxyableRequest(enableByDomain: string, removeBySource?: string, gridRow?: ProxyableDataType) {
